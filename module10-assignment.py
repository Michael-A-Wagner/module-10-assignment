...
df_no_missing["loyalty_status"] = df_no_missing["loyalty_status"].ffill()
...
# ensure last_purchase is datetime (safety)
...
df_no_duplicates["purchase_frequency_category"] = df_no_duplicates["total_purchases"].apply(
...
df_final = df_renamed.drop(columns=["Email", "City", "State"])
...
category_revenue = (
    df_final.groupby("PreferredCategory")["TotalSpent"].sum().sort_values(ascending=False)
)
...
print("- Data Type Issues: join_date, last_purchase, total_spent, total_purchases, age converted to proper types")

# 9.2 Describe the changes made to standardize the dataset
print("\nStandardization Changes:")
print("- Names: Converted to proper case")
print("- Categories: Standardized to title case for PreferredCategory")
